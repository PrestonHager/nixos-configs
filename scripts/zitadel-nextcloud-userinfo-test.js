#!/usr/bin/env node
/** Fetch Zitadel userinfo for prestonh with Nextcloud OAuth scopes. Run on ace as root. */
const fs = require('fs');
...
const OAUTH = '/run/secrets/nextcloud-oidc-env';
...
(async () => {
  ...
  const clientId = load(OAUTH, 'NEXTCLOUD_OIDC_CLIENT_ID=');
  const clientSecret = load(OAUTH, 'NEXTCLOUD_OIDC_CLIENT_SECRET=');
  const scopes = [
    'openid',
    'profile',
    'email',
  ...
  console.log('=== groups claim (user_oidc mapping) ===');
  console.log('userinfo.groups:', JSON.stringify(userinfo.groups));
  console.log('id_token.groups:', JSON.stringify(idClaims.groups));
  console.log('\n=== project roles claim ===');
  const rolesKey = `urn:zitadel:iam:org:project:${PROJECT_ID}:roles`;
  console.log('userinfo roles:', JSON.stringify(userinfo[rolesKey] || userinfo['urn:zitadel:iam:org:project:roles']));
  console.log('id_token roles:', JSON.stringify(idClaims[rolesKey] || idClaims['urn:zitadel:iam:org:project:roles']));
  console.log('\n=== user_oidc expectation ===');
  console.log('mappingGroups=groups, groupWhitelistRegex=/^admin$/, expected groups: ["admin"]');
  const claim = userinfo.groups || idClaims.groups;
  const ok = Array.isArray(claim) && claim.includes('admin');
  console.log(ok ? 'PASS: groups claim includes admin' : 'FAIL: groups claim missing admin');
  process.exit(ok ? 0 : 1);
})().catch((e) => {
