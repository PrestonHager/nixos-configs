...
nix_panel env BLUEPRINT_ENVIRONMENT=ci bash -c "cd '$panel' && rm -f .blueprint/lock && yes n | bash ./blueprint.sh -bash -install sociallogin"
...
install_dev_ext() {
  ...
  nix_panel env BLUEPRINT_ENVIRONMENT=ci bash -c "cd '$panel' && rm -f .blueprint/lock && yes n | bash ./blueprint.sh -bash -install '[developer-build]'"
}
