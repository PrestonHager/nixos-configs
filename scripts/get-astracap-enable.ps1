# Load enable password from Vaultwarden Astracap item (never prints secret).
param(
  [switch]$PreferSecret
)
...
throw 'No enable secret/password found in Astracap Router Info vault item'

# Export for scripts: get-astracap-enable.ps1 -AlsoPassword returns both
