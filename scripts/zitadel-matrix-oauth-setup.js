#!/usr/bin/env node
/**
 * Create/update Matrix OIDC app in Zitadel, matrix_admin role, and admin grant.
 * Run on ace as root (needs login-client key).
 *
 * Prints CLIENT_ID and CLIENT_SECRET for matrix-oauth sops secret.
 */
const fs = require('fs');
...
async function createMatrixApp() {
  ...
  console.log(`Created Matrix OIDC app id=${resp.appId}`);
  return { appId: resp.appId, clientId: resp.clientId, clientSecret: resp.clientSecret };
}

async function updateMatrixApp(appId) {
  ...
  console.log(`Updated Matrix OIDC app ${appId} (redirect + role assertions)`);
...
(async () => {
  ...
  let app = await findMatrixApp();
  let credentials;
  if (!app) {
    credentials = await createMatrixApp();
  } else {
    console.log(`Found existing Matrix app id=${app.id} clientId=${app.clientId || app.oidcConfig?.clientId}`);
    credentials = await updateMatrixApp(app.id);
  }

  await ensureUserGrant();

  console.log('\n=== Matrix OIDC credentials (store in matrix-oauth sops secret) ===');
  console.log(`SYNAPSE_OIDC_CLIENT_ID=${credentials.clientId}`);
  if (credentials.clientSecret) {
    console.log(`SYNAPSE_OIDC_CLIENT_SECRET=${credentials.clientSecret}`);
  } else {
