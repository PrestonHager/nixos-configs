<?php

namespace Pterodactyl\Http\Middleware;

use Closure;
use Ramsey\Uuid\Uuid;
use Illuminate\Http\Request;
use Pterodactyl\Models\User;
use Illuminate\Support\Facades\Auth;
use Symfony\Component\HttpFoundation\Response;

/**
 * Trust reverse-proxy SSO headers (oauth2-proxy + Zitadel OIDC).
 * Based on pterodactyl/panel#5271 with admin group sync via X-Auth-Groups.
 */
class HeaderAuthentication
{
    public function handle(Request $request, Closure $next): Response
    {
        if (!config('auth.header.enabled', false)) {
            return $next($request);
        }

        if (Auth::check()) {
            return $next($request);
        }

        $usernameHeader = config('auth.header.username_header', 'X-Auth-Username');
        $emailHeader = config('auth.header.email_header', 'X-Auth-Email');
        $groupsHeader = config('auth.header.groups_header', 'X-Auth-Groups');
        $adminGroup = config('auth.header.admin_group', 'pterodactyl_admin');

        $username = $request->header($usernameHeader);
        $email = $request->header($emailHeader);

        if (!$username || !$email) {
            return $next($request);
        }

        $groups = array_filter(array_map('trim', explode(',', (string) $request->header($groupsHeader, ''))));
        $isAdmin = in_array($adminGroup, $groups, true);

        $user = User::where('email', $email)->first();

        if (!$user && config('auth.header.auto_create', false)) {
            $user = new User();
            $user->uuid = Uuid::uuid4()->toString();
            $user->username = $username;
            $user->email = $email;
            $user->name_first = $username;
            $user->name_last = $username;
            $user->password = bcrypt(Uuid::uuid4()->toString());
            $user->language = config('app.locale', 'en');
            $user->root_admin = $isAdmin;
            $user->use_totp = false;
            $user->totp_secret = null;
            $user->external_id = '';
            $user->gravatar = true;
            $user->totp_authenticated_at = null;
            $user->save();
        }

        if ($user) {
            if ($user->root_admin !== $isAdmin) {
                $user->root_admin = $isAdmin;
                $user->save();
            }
            Auth::login($user);
        }

        return $next($request);
    }
}
